...
def analytics_stats():
    ...
    try:
        # Check for admin token in header or query parameter
        admin_token = request.headers.get('X-Admin-Token') or request.args.get('token')
        logger.info(f"Analytics stats request - Token: {admin_token}")
        logger.info(f"Request headers: {dict(request.headers)}")
        logger.info(f"Request args: {dict(request.args)}")
        
        # Accept 'flip_ruby' token or allow access without token for demo
        if admin_token and admin_token != 'flip_ruby':
            logger.error(f"Invalid admin token: {admin_token}")
...
def analytics_timeline():
    ...
    try:
        # Check for admin token in header or query parameter
        admin_token = request.headers.get('X-Admin-Token') or request.args.get('token')
        logger.info(f"Timeline request - Token: {admin_token}")
        logger.info(f"Request headers: {dict(request.headers)}")
