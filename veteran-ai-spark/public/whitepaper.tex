...
\fancyhead[R]{\thepage}
\fancyfoot[C]{Technical Whitepaper}
...
\large OpenAI-Powered \textbf{RAG} Architecture with HyDE and Hallucination Prevention}
...
\begin{itemize}
    ...
    \item \textbf{Hallucination Rate}: \textbf{0.01\%} (7-layer prevention system)
...
\begin{tikzpicture}[node distance=1cm, scale=0.85, transform shape]
    ...
    \draw [arrow] (router) -- node[above, sloped, font=\small] {simple} (mini);
    \draw [arrow] (router) -- node[above, sloped, font=\small] {complex} (full);
    \draw [arrow] (mini) -- (guard);
    \draw [arrow] (full) -- (guard);
    \draw [arrow] (guard) -- (response);
    
    % Labels - using \small for better readability
    \node[right=0.3cm of hyde, font=\small] {gpt-4.1-mini};
    \node[right=0.3cm of embed, font=\small] {OpenAI API};
    \node[right=0.3cm of search, font=\small] {In-Memory};
\end{tikzpicture}
...
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[node distance=0.6cm, scale=0.75, transform shape]
    % Traditional path (top)
    \node[font=\small\bfseries] at (-3.5, 3.5) {Traditional:};
    \node (q1) [data, minimum width=2.5cm] at (0, 3.5) {``What is SMC?''};
    \node (e1) [process, right=1cm of q1] {Embed};
    \node (s1) [process, right=1cm of e1] {Search};
    \node (r1) [data, right=1cm of s1, minimum width=1.8cm, fill=red!20] {Score: 0.52};
    ...
    % HyDE path (bottom)
    \node[font=\small\bfseries] at (-3.5, 0.5) {HyDE:};
    \node (q2) [data, minimum width=2.5cm] at (0, 0.5) {``What is SMC?''};
    \node (h2) [process, right=1cm of q2, fill=cyan!30, minimum width=1.5cm] {HyDE};
    
    % Hypothetical document box
    \node (hyp) [draw, below=0.4cm of h2, minimum width=6cm, minimum height=1.3cm, fill=cyan!10, align=left, font=\tiny] {
        \textbf{Hypothetical Document:}\\
        ``SMC is additional compensation for veterans\\
        with severe disabilities under 38 CFR 3.350...''
    };
    
    \node (e2) [process, right=2.5cm of h2] {Embed};
    \node (s2) [process, right=1cm of e2] {Search};
    \node (r2) [data, right=1cm of s2, minimum width=1.8cm, fill=green!30] {Score: 0.77};
...
\centering
\vspace*{\fill}
\begin{tikzpicture}[node distance=1.2cm, scale=1.1, transform shape]
    % Input
    \node (input) [data, minimum width=3.5cm, minimum height=1cm] {User Query};
    
    % Preprocessing
    \node (preprocess) [process, below=of input, minimum width=5cm, minimum height=1cm] {Query Preprocessing};
    \node[right=0.5cm of preprocess, font=\small, align=left] {Expand abbreviations\\(PTSD, SMC, TDIU...)};
    
    % HyDE Generation
    \node (hyde) [process, below=of preprocess, minimum width=5cm, minimum height=1cm, fill=cyan!30] {HyDE Generation};
    \node[right=0.5cm of hyde, font=\small, align=left] {gpt-4.1-mini\\$\sim$200-300 tokens\\$\sim$\$0.00014};
    
    % Hypothetical
    \node (hyp) [draw, below=of hyde, minimum width=6cm, minimum height=1.5cm, fill=cyan!10, font=\normalsize] {Hypothetical Document ($\sim$1000 chars)};
    
    % Embedding
    \node (embed) [process, below=of hyp, minimum width=5cm, minimum height=1cm] {Generate Embedding};
    \node[right=0.5cm of embed, font=\small, align=left] {text-embedding-3-small\\1536 dimensions};
    
    % Vector
    \node (vector) [draw, below=of embed, minimum width=4cm, minimum height=1cm, fill=purple!20, rounded corners] {Query Vector};
    
    % Search
    \node (search) [process, below=of vector, minimum width=5cm, minimum height=1cm] {Cosine Similarity Search};
    
    % Results
    \node (results) [data, below=of search, minimum width=3.5cm, minimum height=1cm] {Top-K Chunks};
    ...
    % Corpus annotation
    \node (corpus) [draw, right=2.5cm of search, minimum width=2.5cm, minimum height=1cm, fill=green!20, rounded corners, align=center] {Corpus\\(1468 chunks)};
...
\begin{axis}[
    ...
    xtick=data,
    legend style={font=\small, at={(1.02,0.5)}, anchor=west},
    nodes near coords,
...
\begin{enumerate}
    \item \textbf{Semantic Enrichment}: The hypothetical document contains domain-specific terms (CFR references, rating percentages, forms) that match corpus vocabulary.
...
\vspace{0.3cm}
\begin{tikzpicture}
    \node[draw, fill=red!60, minimum width=0.5cm, minimum height=0.4cm] at (0,0) {};
    \node[right, font=\small] at (0.4, 0) {Refused ($<0.45$)};
    \node[draw, fill=yellow!60, minimum width=0.5cm, minimum height=0.4cm] at (4,0) {};
    \node[right, font=\small] at (4.4, 0) {Flagged ($0.45$-$0.50$)};
    \node[draw, fill=orange!60, minimum width=0.5cm, minimum height=0.4cm] at (0,-0.6) {};
    \node[right, font=\small] at (0.4, -0.6) {Warning ($0.50$-$0.55$)};
    \node[draw, fill=green!60, minimum width=0.5cm, minimum height=0.4cm] at (4,-0.6) {};
    \node[right, font=\small] at (4.4, -0.6) {Accepted ($>0.55$)};
\end{tikzpicture}
...
\begin{lstlisting}[caption=Citation Cleanup, basicstyle=\ttfamily\scriptsize]
def clean_hallucinated_citations(response, max_valid):
    """Remove citations referencing non-existent sources."""
    cleaned = response
    # Remove superscript citations beyond valid range
    for i in range(max_valid + 1, 20):
        sup = NUM_TO_SUPERSCRIPT.get(i, '')
        if sup in cleaned:
            cleaned = cleaned.replace(sup, '')
    # Remove [N] bracket citations for non-existent sources
    def replace_invalid(m):
        return '' if int(m.group(1)) > max_valid else m.group(0)
    return re.sub(r'\[(\d+)\]', replace_invalid, cleaned)
\end{lstlisting}
...
\begin{axis}[
    ...
    legend style={font=\small, at={(1.02,0.5)}, anchor=west},
    grid=major,
    title={Hallucination Rate: Progressive Improvement Over 40 Weeks}
]
\addplot[thick, mark=square*, blue, mark repeat=2] coordinates {
    (1, 12) (2, 11.5) (3, 11) (4, 10.5) (5, 10) (6, 9.5) (7, 9) (8, 8.5)
};
\addplot[thick, mark=*, red, mark repeat=2] coordinates {
    (8, 8.5) (10, 6) (12, 4.5) (14, 3.8) (16, 3.2) (18, 2.8) (20, 2.5)
};
\addplot[thick, mark=triangle*, green!70!black, mark repeat=2] coordinates {
    (20, 2.5) (22, 1.8) (24, 1.2) (26, 0.8) (28, 0.5) (30, 0.3) (32, 0.15) (34, 0.08) (36, 0.04) (38, 0.02) (40, 0.01)
};
\legend{Baseline, 4-Layer System, 7-Layer System}
\draw[dashed, thick] (axis cs:8,0) -- (axis cs:8,15);
\draw[dashed, thick, green!70!black] (axis cs:20,0) -- (axis cs:20,15);
\node at (axis cs:8, 13) [anchor=south, font=\tiny] {4-Layer};
\node at (axis cs:20, 13) [anchor=south, font=\tiny] {7-Layer};
\end{axis}
\end{tikzpicture}
\caption{Hallucination Rate: 7-Layer System Achieves 0.01\% Rate}
\end{figure}
...
\midrule
Hallucination Rate & 12\% & 3.2\% & \textbf{0.01\%} \\
...
\newpage
\section{Knowledge Graph for Cache Lookups}
...
\textbf{Entities} & DC 6260, VA Form 21-526EZ & Specific codes, forms, benefits mentioned \\
\bottomrule
\end{tabular}
...
\begin{tikzpicture}[scale=0.85, transform shape]
    % Input query
    \node[draw, rounded corners, fill=gray!20, minimum width=2.5cm, minimum height=0.8cm] (query) at (-5, 1) {Query};
    
    % L1
    \node[draw, minimum width=7cm, minimum height=1cm, fill=green!30, rounded corners] (l1) at (0, 2) {\textbf{L1: In-Memory Cache}};
    \node[font=\footnotesize, gray] at (0, 1.3) {Exact + Semantic Match};
    
    % L2
    \node[draw, minimum width=7cm, minimum height=1cm, fill=blue!30, rounded corners] (l2) at (0, 0) {\textbf{L2: PostgreSQL Cache}};
    \node[font=\footnotesize, gray] at (0, -0.7) {Database Persistence};
    
    % L3
    \node[draw, minimum width=7cm, minimum height=1cm, fill=purple!30, rounded corners] (l3) at (0, -2) {\textbf{L3: Knowledge Graph}};
    \node[font=\footnotesize, gray] at (0, -2.7) {Topic + Entity Match};
    
    % LLM
    \node[draw, rounded corners, fill=red!20, minimum width=2.5cm, minimum height=0.8cm] (llm) at (0, -4) {LLM Generation};
    
    % Arrows
    \draw[thick, ->] (query) -- (l1.west);
    \draw[thick, ->] (l1.south) -- (l2.north) node[midway, right, font=\footnotesize] {Miss};
    \draw[thick, ->] (l2.south) -- (l3.north) node[midway, right, font=\footnotesize] {Miss};
    \draw[thick, ->] (l3.south) -- (llm.north) node[midway, right, font=\footnotesize] {Miss};
    
    % Hit rates on right side
    \node[font=\footnotesize, fill=green!10, rounded corners, draw=green!50] at (5, 2) {35\% hit rate};
    \node[font=\footnotesize, fill=blue!10, rounded corners, draw=blue!50] at (5, 0) {15\% hit rate};
    \node[font=\footnotesize, fill=purple!10, rounded corners, draw=purple!50] at (5, -2) {10\% hit rate};
    
    % Connecting lines to hit rates
    \draw[dotted, gray] (l1.east) -- (4, 2);
    \draw[dotted, gray] (l2.east) -- (4, 0);
    \draw[dotted, gray] (l3.east) -- (4, -2);
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[
    layer/.style={draw, minimum width=6.5cm, minimum height=0.7cm, rounded corners=3pt, font=\small},
    component/.style={draw, minimum width=1.8cm, minimum height=0.5cm, rounded corners=2pt, font=\scriptsize},
    scale=0.9, transform shape
]
    % Title/User
    \node[layer, fill=gray!20] (user) at (0, 5.5) {\textbf{User Browser}};
    
    % Frontend
    \node[layer, fill=blue!20] (frontend) at (0, 4.3) {React SPA};
    
    % API
    \node[layer, fill=green!20] (api) at (0, 3.1) {Flask API + Gunicorn};
    
    % RAG Core box
    \node[draw, dashed, minimum width=7cm, minimum height=2.8cm, rounded corners=5pt, fill=orange!5] (ragbox) at (0, 1) {};
    \node[font=\small\bfseries, orange!70!black] at (0, 2.2) {RAG Pipeline};
    
    % Inside RAG box
    \node[component, fill=cyan!30] (hyde) at (-2, 1.3) {HyDE};
    \node[component, fill=purple!20] (vector) at (0, 1.3) {Vector Store};
    \node[component, fill=purple!20] (embed) at (2, 1.3) {Embeddings};
    \node[component, fill=yellow!30] (router) at (-1, 0.3) {Model Router};
    \node[component, fill=red!20] (guard) at (1, 0.3) {7-Layer Guard};
    
    % Cache
    \node[layer, fill=blue!15, minimum width=6.5cm] (cache) at (0, -0.9) {Response Cache (L1 + L2 + L3)};
    
    % External Services
    \node[draw, minimum width=2.5cm, minimum height=0.6cm, fill=red!30, rounded corners=3pt, font=\small] (openai) at (-2, -2.2) {OpenAI API};
    \node[draw, minimum width=2.5cm, minimum height=0.6cm, fill=blue!30, rounded corners=3pt, font=\small] (postgres) at (2, -2.2) {PostgreSQL};
    
    % Arrows - main flow
    \draw[arrow] (user) -- (frontend);
    \draw[arrow] (frontend) -- (api);
    \draw[arrow] (api) -- (ragbox.north);
    
    % Internal RAG arrows
    \draw[arrow, gray] (hyde) -- (vector);
    \draw[arrow, gray] (vector) -- (embed);
    \draw[arrow, gray] (hyde) -- (router);
    \draw[arrow, gray] (router) -- (guard);
    
    % Output arrows
    \draw[arrow] (ragbox.south) -- (cache);
    \draw[arrow] (cache) -- (openai);
    \draw[arrow] (cache) -- (postgres);
    
    % Labels on right
    \node[font=\tiny, gray, right] at (3.8, 5.5) {HTTPS};
    \node[font=\tiny, gray, right] at (3.8, 4.3) {Static Assets};
    \node[font=\tiny, gray, right] at (3.8, 3.1) {REST API};
    \node[font=\tiny, gray, right] at (3.8, -2.2) {External};
\end{tikzpicture}
...
\begin{axis}[
    ...
    ymin=0,
    ymax=350,
    symbolic x coords={HNSW m=16, No Index, HNSW m=24},
    xtick=data,
...
\addplot[fill=green!60] coordinates {
    (HNSW m=16, 48.2) (No Index, 91.6) (HNSW m=24, 93.7)
};
\addplot[fill=orange!60] coordinates {
    (HNSW m=16, 59.7) (No Index, 130.3) (HNSW m=24, 197.5)
};
\addplot[fill=red!60] coordinates {
    (HNSW m=16, 95.9) (No Index, 195.3) (HNSW m=24, 302.5)
};
...
\addplot[fill=blue!60] coordinates {
    (HNSW m=16, 44.7) (No Index, 9.6) (HNSW m=24, 8.6)
};
\end{axis}
\end{tikzpicture}
...
\begin{itemize}
    ...
    \item \textbf{0.01\% hallucination rate} (down from 12\%) with 7-layer prevention system
